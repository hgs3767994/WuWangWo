...
- 原生 App 只取得一小時的 Worker access session，以及只能換發短效 session、最長 30 天且每次使用都輪替的裝置續期憑證；Google access token、Google refresh token、DEK、主密碼與救援碼不得進入 Web bundle、Android resources、iOS plist 或 Git。
...
- Worker 的 native exchange endpoint只接受一次性 server auth code，並以伺服器端 Web client secret完成交換後建立一小時 access session 與可撤銷的裝置續期憑證。Android OAuth client仍以 package name與簽章 SHA-1識別正式 App。
