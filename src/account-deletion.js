...
const deleteDrive = document.querySelector("[data-delete-drive]");
...
button?.addEventListener("click", async () => {
  ...
  try {
    const result = await deleteGoogleCloudAccount({ deleteDriveData: deleteDrive.checked, popupWindow });
    confirmation.value = "";
    deleteDrive.disabled = true;
    confirmation.disabled = true;
    button.textContent = "刪除完成";
    showStatus(result.driveDataDeleted
      ? "刪除完成：雲端帳號、Google 授權與莫忘 Drive 加密同步檔都已永久刪除。"
      : "刪除完成：雲端帳號與 Google 授權已永久刪除；Google Drive 加密同步檔依你的選擇保留。", false);
  } catch (error) {
