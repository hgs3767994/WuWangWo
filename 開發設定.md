...
啟動時讀取 IndexedDB、本機加密資料、登入狀態或快照若失敗，App 會顯示安全復原畫面，而非空白頁面。此畫面不會清除、覆寫或同步資料，並會顯示診斷代碼：`BOOT-APP-STATE`、`BOOT-APP-STATE-WRITE`、`BOOT-KEY-PACKAGE`、`BOOT-LOCAL-VAULT`、`BOOT-TRUSTED-SESSION`、`BOOT-LOCAL-SNAPSHOTS`、`BOOT-SESSION-CHECK`、`BOOT-SESSION-RESTORE` 或 `BOOT-INITIAL-SAVE`。
