...
### v181 PWA 原子更新保護（2026-09-20）

- [x] 重現新版 Service Worker 已提示更新，但 Pages/CDN 尚未同步全部靜態檔案，導致新版快取名稱中實際保存舊版程式的問題。
- [x] App shell 預快取改用版本化、`no-store` 請求，並在安裝階段核對 `config.js` 的 cache 版本必須與 Service Worker 完全相同。
- [x] 偵測到混合版本或任一檔案下載失敗時，會刪除不完整的新快取並中止安裝，保留目前可用版本等待下次更新重試。
- [x] PWA cache 更新為 v181；不需清除網站資料。

### v182 OAuth 返回歷史隔離（2026-09-20）

- [x] 重現 PWA 完成 OAuth、解鎖並返回設定頁後，設定頁「首頁」按鈕會依序退回 Google 帳號選擇器與登入頁的問題。
- [x] 根因為 OAuth 回跳先進入主密碼解鎖分支，原有的 Google history barrier 尚未安裝就提前結束啟動流程。
